...
**Проблема:** Блок создания коллекции/окружения (строки 39-84 в `generate-tests.ts`) падал в `catch` из-за проблемы #2 (протухшее DB-соединение). Ошибка тихо проглатывалась — `collection` и `environment` отсутствовали в ответе.

**Исправление:** Исправлен `getDb()` (проблема #2). Добавлено логирование ошибок в `catch` вместо тихого игнорирования.

**Файлы:** `src/mcp/tools/generate-tests.ts`, `src/db/schema.ts`
